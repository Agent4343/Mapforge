...
def generate_svg(
    ...
    layer_count = 3 + (1 if show_coordinates else 0)  # outline/fill, text, coords, board
    ...
    lines.append(f"  <!-- MapForge CNC v1.0 -->")
    ...
    lines.append(f"  <!-- Geographic data: © OpenStreetMap contributors (ODbL) -->")
    lines.append(f"  <!-- Canadian topo data: Natural Resources Canada, Open Government Licence -->")
    ...
    if style == CutStyle.outline:
        ...
        for exterior, holes in polygons:
            # Combine exterior and holes into a single path with fill-rule
            path_d = _coords_to_path(exterior)
            for hole in holes:
        ...
        lines.append("  <!-- Layer: geography_outline -->")
        lines.append('  <!-- Toolpath: V-Carve, 60° V-bit -->')
        lines.append('  <g id="geography_outline">')
        for exterior, holes in polygons:
            path_d = _coords_to_path(exterior)
            lines.append(
    ...
    lines.append('  <!-- Toolpath: V-carve, 60° V-bit, flat depth 0.05" -->')
    ...
    if show_coordinates and latlon:
        ...
        lines.append('  <!-- Toolpath: V-carve, 60° V-bit, flat depth 0.03" -->')
...
def _coords_to_path(coords: list[tuple]) -> str:
    """Convert coordinate list to SVG path d attribute (M/L/Z only)."""
    if not coords:
        return ""

    parts = [f"M{coords[0][0]},{coords[0][1]}"]
    for x, y in coords[1:-1]:  # skip last point (same as first for closed ring)
